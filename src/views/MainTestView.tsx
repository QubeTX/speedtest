...
import { useInstrumentReset } from '../hooks/useInstrumentReset';
import './instrument-v5.css';

export default function MainTestView() {
  const { phase: runPhase, progress, result: runResult, settings, updateSettings, startTest, stopTest, resetTest } = useSpeedTestContext();
  const { instrumentRef, beginReset, resetting, resetStage, retainedResult } = useInstrumentReset(resetTest);
  const result = retainedResult ?? runResult;
  const phase = retainedResult ? 'complete' : runPhase;
  ...
  const action = () => {
    if (resetting) return;
    if (complete && result) { beginReset(result); return; }
    setDetails(false);
    if (active) stopTest(); else void startTest(profile);
  };
  ...
  return <main ref={instrumentRef} className={`v5-instrument${resetStage ? ` v5-reset--${resetStage}` : ''}`}>
    ...
    <div className="v5-body">
      <section className="v5-deck" aria-label="Test transport">
        <div className="v5-status" role="status"><strong>{resetting ? 'Rewinding to ready' : status}</strong><span>{active ? progress.currentProvider : profile === 'fast' ? 'Quick / 90 s max' : 'Deep / 5 min max'}</span></div>
        <TapeMechanism phase={phase} currentSpeed={speed} downloadProgress={progress.downloadProgress} uploadProgress={progress.uploadProgress} onPress={action} resetting={resetting} />
        <div className="v5-modes" role="radiogroup" aria-label="Measurement profile">{(['fast', 'full'] as const).map(mode => <button key={mode} role="radio" aria-checked={profile === mode} disabled={active || resetting} onClick={() => updateSettings({ testProfile: mode })}>{mode === 'fast' ? 'QUICK' : 'DEEP'}<small>{mode === 'fast' ? '90 SEC MAX' : '5 MIN MAX'}</small></button>)}</div>
      ...
      <section className="v5-readout" aria-label="Measurement results" aria-busy={resetting}>
        ...
        <div className="v5-metrics">
          {cells.map(cell => { const formatted = cell.value == null ? null : formatSpeed(cell.value, settings.speedUnit), ceiling = cell.ceiling == null ? null : formatSpeed(cell.ceiling, settings.speedUnit); return <button className="v5-metric" key={cell.label} onClick={() => setMetricInfo(cell.label)} aria-label={`${cell.label.toLowerCase()}, ${formatted ? `${formatted.value} ${formatted.unit}` : 'no reading'}. Show metric explanation`}><span className="v5-metric-label">{cell.label}</span><span className="v5-value"><strong style={{ "--reading-digits": formatted?.value.length ?? 1 } as CSSProperties}>{formatted?.value ?? '—'}</strong><span>{formatted?.unit ?? 'Mbps'}</span></span><small>{ceiling ? `Est. ceiling ${ceiling.value} ${ceiling.unit}` : complete ? 'Ceiling not established' : 'Ceiling available after the test'}</small></button>; })}
          {[{ label: 'PING', value: ping, note: 'Median idle HTTP RTT' }, { label: 'JITTER', value: jitter, note: 'Idle RTT · P95 − P50' }].map(cell => <button className="v5-metric" key={cell.label} onClick={() => setMetricInfo(cell.label)} aria-label={`${cell.label.toLowerCase()}, ${cell.value == null ? 'no reading' : `${cell.value.toFixed(1)} ms`}. Show metric explanation`}><span className="v5-metric-label">{cell.label}</span><span className="v5-value"><strong style={{ "--reading-digits": cell.value == null ? 1 : cell.value.toFixed(1).length } as CSSProperties}>{cell.value == null ? '—' : cell.value.toFixed(1)}</strong><span>ms</span></span><small>{cell.note}</small></button>)}
        </div>

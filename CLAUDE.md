...
### Layout
- `MainTestView` and `instrument-v5.css` provide the current cassette/measurement layout. The stylesheet's final native-continuity section shares the app's neutral surfaces, white grouped readings and Makira hierarchy. `cassette-shell.ts` reduces the iOS silhouette to a rounded blue-gray body, darker window, tape link and broad black transport. `TapeMechanism` positions the larger reels in its 360-by-200 housing; internal reel geometry and motion remain shared. Older apparatus/data panels are retained historical components.
